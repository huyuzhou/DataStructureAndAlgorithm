// stack-list.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
p_list_stack_node list_stack_node_create(int key);
...
bool list_stack_push(p_list_stack liststack, int key);
...
//入栈
bool list_stack_push(p_list_stack liststack, int key) {
	p_list_stack_node newnode = list_stack_node_create(key);
	assert(newnode);
...
void main()
{
	...
	list_stack_push(&liststack, 1);
	list_stack_push(&liststack, 2);
	list_stack_push(&liststack, 3);
	list_stack_push(&liststack, 4);
	list_stack_push(&liststack, 5);
	printf("链栈的初始内容为：");
