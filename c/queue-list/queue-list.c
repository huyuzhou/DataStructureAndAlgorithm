// queue-list.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
p_list_queue_node list_queue_node_create(int key);
...
bool list_queue_in(p_list_queue listqueue, int key);
...
//入队
bool list_queue_in(p_list_queue listqueue, int key) {
	p_list_queue_node newnode = list_queue_node_create(key);
...
int main()
{
	...
	list_queue_in(&listqueue, 1);
	list_queue_in(&listqueue, 2);
	list_queue_in(&listqueue, 3);
	list_queue_in(&listqueue, 4);
	list_queue_in(&listqueue, 5);
	list_queue_in(&listqueue, 6);
	printf("初始队列为：");
	...
	list_queue_display(&listqueue);
	printf("再入队2个元素：");
	list_queue_in(&listqueue, 11);
	list_queue_in(&listqueue, 12);
	list_queue_display(&listqueue);
	printf("当前队列长度为：");
